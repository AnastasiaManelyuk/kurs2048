...
export const useGame = () => {
  ...
  // Необходимо сохранить анимацию скольжения, если действие объединяет плитки вместе.
  ...
  const move = (
    ...
    // новые плитки не могут быть созданы во время движения.
    dispatch({ type: "START_MOVE" });

    const maxIndex = tileCountPerRowOrColumn - 1;

    // перебирает каждую строку или столбец (зависит от вида перемещения - вертикального или горизонтального).
    for (
      let rowOrColumnIndex = 0;
      rowOrColumnIndex < tileCountPerRowOrColumn;
      rowOrColumnIndex += 1
    ) {
      // извлекает плитки в строке или столбце.
      const availableTileIds = retrieveTileIdsPerRowOrColumn(rowOrColumnIndex);

      // предыдущая плитка используется для определения того, можно ли объединить плитку с текущей плиткой.
      let previousTile: TileMeta | undefined;
      // помогает заполнить пробелы, созданные слиянием плиток - две плитки становятся одной.
      let mergedTilesCount = 0;

      // перебирайте доступные плитки.
      availableTileIds.forEach((tileId, nonEmptyTileIndex) => {
        const currentTile = tiles[tileId];

        // если предыдущий файл имеет то же значение, что и текущий, они должны быть объединены вместе.
        if (
          ...
          // задерживает слияние на 250 мс, поэтому анимация скольжения может быть завершена.
          throttledMergeTile(tile, previousTile);
          // предыдущая плитка должна быть очищена, так как одна плитка может быть объединена только один раз за ход.
          previousTile = undefined;
          // увеличьте объединенный счетчик, чтобы исправить положение последовательных плиток, чтобы избавиться от пробелов
          mergedTilesCount += 1;

          return updateTile(tile);
        }

        // в противном случае - предыдущие и текущие плитки отличаются - переместите плитку на первое свободное место.
        const tile = {
          ...currentTile,
        ...
        // предыдущая плитка становится текущей плиткой, чтобы проверить, можно ли объединить следующую плитку с этой.
        previousTile = tile;

        // только в том случае, если плитка изменила свое положение, она будет обновлена
    ...
    // дождитесь окончания всех анимаций.
